    ...
    public function optimize($tables = null){
        if($tables) {
            ...
            if(is_array($tables)){
                ...
                if($list){
                    $this->ajaxReturn(['info'=>'数据表优化完成!','status'=>1]);
                } else {
                    $this->ajaxReturn(['info'=>'数据表优化出错请重试!','status'=>0]);
                }
            } else {
                $list = $Db->query("OPTIMIZE TABLE `{$tables}`");
                if($list){
                    //$this->success("数据表'{$tables}'优化完成!");
                    $this->ajaxReturn(['info'=>"数据表'{$tables}'优化完成!",'status'=>0]);
                } else {
                    //$this->error("数据表'{$tables}'优化出错请重试!");
                    $this->ajaxReturn(['info'=>"数据表'{$tables}'优化出错请重试!",'status'=>0]);
                }
            }
        } else {
            //$this->error("请指定要优化的表!");
            $this->ajaxReturn(['info'=>"请指定要优化的表!",'status'=>1]);
        }
    }
        ...
        if($tables) {
            ...
            if(is_array($tables)){
                ...
                if($list){
                    ...
                    $this->ajaxReturn(['info'=>"数据表修复完成!",'status'=>1]);
                } else {
                    //$this->error("数据表修复出错请重试!");
                    $this->ajaxReturn(['info'=>"数据表修复出错请重试!",'status'=>0]);
                }
            } else {
                $list = $Db->query("REPAIR TABLE `{$tables}`");
                if($list){
                    //$this->success("数据表'{$tables}'修复完成!");
                    $this->ajaxReturn(['info'=>"数据表'{$tables}'修复完成!",'status'=>0]);
                } else {
                    //$this->error("数据表'{$tables}'修复出错请重试!");
                    $this->ajaxReturn(['info'=>"数据表'{$tables}'修复出错请重试!",'status'=>0]);
                }
            }
        } else {
            //$this->error("请指定要修复的表!");
            $this->ajaxReturn(['info'=>"请指定要修复的表!",'status'=>0]);
        }
    }
    ...
    public function del($time = 0){
        if($time){
            ...
            if(count(glob($path))){
                ...
                $this->ajaxReturn(['info'=>'备份文件删除失败，请检查权限!','status'=>0]);
            } else {
                $this->ajaxReturn(['info'=>'备份文件删除成功!','status'=>1]);
                //$this->success('备份文件删除成功!');
            }
        } else {
            //$this->error('参数错误!');
            $this->ajaxReturn(['info'=>'参数错误!','status'=>0]);
        }
    }
